...
Loader {
  ...
  sourceComponent: Component {
    PanelWindow {
      ...
      anchors.right: true
      anchors.bottom: true
      margins.left: (barIsVisible && !barAtBottom) ? (barHeight + ((Settings.data.bar.floating && !panelAnchorVerticalCenter) ? Settings.data.bar.marginTop : 0)) : 0
      margins.right: (barIsVisible && barAtBottom) ? (barHeight + ((Settings.data.bar.floating && !panelAnchorVerticalCenter) ? Settings.data.bar.marginBottom : 0)) : 0
